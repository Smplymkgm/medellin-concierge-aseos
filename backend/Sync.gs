...
function sincronizarCalendarios() {
  ...
  for (var p of propiedades) {
    ...
    reservas = reservas.concat(obtenerReservasDeICal(p));
  }
...
// ============================================================
// SINCRONIZAR HOJA ASEOS (desde master)
// ============================================================
...
function sincronizarHojaAseos() {
  ...
  for (var i = 0; i < mDatos.length; i++) {
    ...
    if (existentes[cod]) {
      ...
      if (nuevoEstado === "Completado" && !hoja.getRange(fila, 13).getValue()) {
  ...
  if (nuevas.length > 0) {
    ...
    for (var i = 0; i < nuevas.length; i++) {
      var bg = nuevas[i][7] === "Completado" ? "#e8f5e9" :
